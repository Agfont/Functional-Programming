  ...
  (type-case ExprC a
      ...
      ;      (interp (closV-body f-value)
       ;             (extend-env
        ;                (bind (closV-arg f-value) (box (interp a env)))
         ;               (closV-env f-value)
          ;          )))]
    [classC (name var m1 m2) (classV name var (interp m1 env) (interp m2 env))]
    [sendC (obj name arg)
    ...
    [newC  (name value)
          (let* ([class (unbox (lookup name env))]
            ...
            (objectV name (extend-env (bind (classV-var class) (box (interp value (objectV-env parent)))) (objectV-env parent))))]
...
; Facilitator
(define (interpS [s : s-expression]) (interp (desugar (parse s)) create-env))
